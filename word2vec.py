...
def naiveSoftmaxLossAndGradient(
    ...
    # Step 1: 외부 단어 벡터들과 내부 단어 벡터의 내적을 통해 softmax 함수를 통과
    y_hat = softmax(np.dot(outsideVectors, centerWordVec))
    
    # Step 2: 크로스 엔트로피 손실 계산
    loss = -np.log(y_hat[outsideWordIdx])
    
    # Step 3: 그라디언트계산
    y_hat[outsideWordIdx] -= 1  # Subtract 1 from the probability of the true outside word
    gradCenterVec = np.dot(outsideVectors.T, y_hat)
    
    # Step 4: Compute the gradient with respect to the outside word vectors
...
def negSamplingLossAndGradient(
    ...
    for i, negIdx in enumerate(negSampleWordIndices):
        ...
        loss += -np.log(sigmoid(-score))
        gradCenterVec += sigmoid(-score) * u_k
        gradOutsideVecs[negIdx] += sigmoid(-score) * centerWordVec
